...
export class LedgerRepository extends BaseRepository {
  ...
  async computeBalance(walletId: string, tx?: Prisma.TransactionClient): Promise<bigint> {
      const client = tx ?? this.prisma;
    const [credits, debits] = await Promise.all([
      client.journalEntry.aggregate({
    ...
    return (
      ((credits._sum.amount ?? 0n) as bigint) -
      ((debits._sum.amount ?? 0n) as bigint)
    );
