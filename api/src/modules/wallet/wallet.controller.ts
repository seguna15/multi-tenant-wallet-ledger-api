...
import { CreateWalletDto, ListWalletsQueryDto, ResolveWalletQueryDto } from '@modules/wallet/dto';
...
export class WalletController {
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: "List the authenticated user's wallets (cursor-paginated)",
  })
  @ApiOkResponse({
    description: 'Paginated list of wallets belonging to the current user',
  })
  ...
  @ApiOperation({
    summary:
      'Resolve an account number to a wallet ID — tenant-scoped; 404 for unknown or cross-tenant numbers',
  })
  @ApiOkResponse({
    description: 'Resolved wallet ID, account number, and currency',
  })
  resolveWallet(@Query() query: ResolveWalletQueryDto) {
    return this.walletService.resolveByAccountNumber(query.accountNumber);
  }
