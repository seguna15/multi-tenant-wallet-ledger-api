...
export class AuthService {
  ...
  async dashboardLogin(dto: LoginDto): Promise<TokenPair> {
    try {
      const user = await this.authRepository.findAdminUserByEmail(dto.email);
  ...
  async refreshToken(incomingToken: string): Promise<TokenPair> {
    try {
      ...
      if (stored.revoked) {
        await this.authRepository.revokeFamilyByFamilyId(
          stored.familyId,
          stored.tenantId,
        );
  ...
  async logoutAll(userId: string, tenantId: string): Promise<void> {
    try {
      await this.authRepository.revokeAllRefreshTokensForUser(userId, tenantId);
    } catch {
