...
const mockUser = (overrides: Partial<User> = {}): User => ({
  ...
  role: UserRole.TENANT_ADMIN, // role is now a required field on User
  isActive: true,
  createdAt: new Date(),
  updatedAt: new Date(),
  ...overrides,
...
describe('AuthService', () => {
  ...
  describe('login', () => {
    ...
    it('signs the access token with sub, email, and tenantId', async () => {
      ...
      // role is now part of the JWT payload so guards can do role checks without a DB lookup
  ...
  describe('refreshToken', () => {
    ...
    it('throws ForbiddenException when token is not found', async () => {
      ...
      // An unrecognized token is treated as a forbidden access attempt, not just "unauthenticated"
    ...
    it('revokes the token family and throws on reuse of a revoked token', async () => {
      ...
      // Family revocation is tenant-scoped to avoid cross-tenant token collisions
    ...
    it('revokes the expired token and throws ForbiddenException', async () => {
      authRepo.findRefreshTokenByHash.mockResolvedValue(
  ...
  describe('logoutAll', () => {
    it('revokes all refresh tokens for the user', async () => {
      ...
      // logoutAll is tenant-scoped, so it now takes both userId and tenantId
      await service.logoutAll('user-uuid-1', 'tenant-uuid-1');
    ...
    it('throws InternalServerErrorException on unexpected repository error', async () => {
      ...
      await expect(
        service.logoutAll('user-uuid-1', 'tenant-uuid-1'),
      ).rejects.toThrow(InternalServerErrorException);
    });
  });
});
