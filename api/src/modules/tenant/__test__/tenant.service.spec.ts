...
// jest maps @prisma-client to the generated client, matching the app's runtime import
...
jest.mock('@shared/utils/api-key.util', () => ({
  ...
  hashApiKey: jest.fn(),
  // generateWebhookSecret is also used by the service when a webhookUrl is provided
  generateWebhookSecret: jest.fn(),
}));

// encrypt() reads WEBHOOK_ENCRYPTION_KEY from the environment — mock it so the
// test doesn't depend on that env var being set
...
import {
  ...
  hashApiKey,
} from '@shared/utils/api-key.util';
...
const mockGenerateWebhookSecret = generateWebhookSecret as jest.MockedFunction<
  typeof generateWebhookSecret
>;
...
const mockTenant = (overrides: Partial<Tenant> = {}): Tenant => ({
  ...
  name: 'Acme Corp',
  type: TenantType.CLIENT, // type is now a required field on Tenant
...
describe('TenantService', () => {
  ...
  describe('createSingleTenant', () => {
    it('returns the created tenant and the plaintext API key', async () => {
      ...
      repo.createSingleTenant.mockResolvedValue(tenant);

      const result = await service.createSingleTenant({ name: 'Acme Corp' });

      // the service strips apiKeyHash before returning the tenant
      const { apiKeyHash: _, ...expectedTenant } = tenant;
    ...
    it('forwards optional webhook fields to the repository', async () => {
      repo.createSingleTenant.mockResolvedValue(mockTenant());
      mockGenerateWebhookSecret.mockReturnValue('plain_webhook_secret');
      mockEncrypt.mockReturnValue('encrypted_webhook_secret');
      ...
      // the webhook secret is stored encrypted, not in plaintext
  ...
  describe('rotateApiKey', () => {
    ...
    it('stores the new hash, not the plaintext key', async () => {
      ...
      // rotateApiKey also receives a new expiry date for the rotated key
